...
var LERC = function () {
  ...
  var parse_ = function(buffer) {
    ...
    pixelValuesDataView_ = new DataView(headerInfo_.height * headerInfo_.width * 4);
  ...
  var readTiles_ = function (buffer) {
    if (headerInfo_.version > 1 &&
      ...
      throw "Try Huffman look up for table is not implemented yet";
    }
        ...
        for (var i = i0; i < i1; i++) {
          ...
          for (var j = j0; j < j1; j++, k++) {
            ...
            if (headerInfo_.lercDataType === DataType.FLOAT) {
              pixelValuesDataView_.setFloat32(k, offset);
            } else {
  ...
  var getDataTypeUsed_ = function(tc) {
    console.log("Get data type used " + tc);
  ...
  var bitStuff2Decode_ = function(buffer, lerc2Version) {
    ...
    if (!doLut) {
      ...
      throw "BitStuff2 decode failed because of unsupported argv";
    } else {
  ...
  var bitStuff2Unstuff_ = function(buffer, numElements, numBits) {
    ...
    var numUInts = (numElements * numBits + 31) / 32;
